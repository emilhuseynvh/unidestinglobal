  ...
  return (
    <div className="flex flex-col items-center gap-3 shrink-0">
      <div className="relative size-16">
        ...
        <span className="absolute inset-0 flex items-center justify-center text-[14px] font-semibold text-[#181d27] leading-5">
          {percent}%
        </span>
      </div>
      <span className={`text-[14px] font-semibold leading-5 ${percent >= 100 ? "text-[#f04438]" : "text-[#007aff]"}`}>Booked</span>
    </div>
  )
}
...
const RepeatIcon = () => (
  <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M12 1.333l2 2-2 2M2 7.333V6a2.667 2.667 0 012.667-2.667h9.333M4 14.667l-2-2 2-2M14 8.667V10A2.667 2.667 0 0111.333 12.667H2" stroke="#808080" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/></svg>
)
const UsersIcon = () => (
  ...
  return (
    <div className="border border-[#e6e6e6] rounded-2xl p-3 sm:p-4 flex flex-col gap-3">
      <div className="flex flex-col sm:flex-row items-start justify-between gap-3 sm:gap-4">
        <div className="flex gap-3 sm:gap-4 items-start flex-1 min-w-0 w-full">
          ...
          <div className="hidden sm:flex flex-1 flex-col gap-3 min-w-0">
            ...
            <div className="flex items-center gap-1">
              <img src={course.instructor.avatar} alt="" className="size-8 rounded-full object-cover" />
              <span className="text-[14px] font-normal text-[#333] leading-[1.4]">{course.instructor.name}</span>
            </div>

            <div className="flex flex-wrap items-center gap-1">
              <span className={`px-1.5 py-0.5 rounded-md text-[12px] font-medium border leading-[18px] ${tagStyles[course.type]}`}>{course.type}</span>
              <span className={`px-1.5 py-0.5 rounded-md text-[12px] font-medium border leading-[18px] ${tagStyles[course.mode]}`}>{course.mode}</span>
              {status && (
                <span className={`flex items-center gap-1 ${status.bg} border ${status.border} rounded-md px-1.5 py-0.5 text-[12px] font-medium ${status.text} leading-[18px]`}>
                  <span className={`size-1.5 rounded-full ${status.dot}`} />
                  {course.status}
                </span>
              )}
              {course.daysLeft && (
                <span className="flex items-center gap-1 bg-[#fef3f2] border border-[#fecdca] rounded-md pl-1 pr-1.5 py-0.5 text-[12px] font-medium text-[#b42318] leading-[18px]">
                  <AlertIcon />
                  {course.daysLeft}
                </span>
              )}
            ...
            <div className="flex flex-wrap items-center gap-4 text-[14px] font-medium text-[#808080] leading-[1.4]">
        ...
        <div className="flex flex-col items-end gap-[59px] shrink-0">
          <span className="text-[20px] font-semibold text-black leading-[1.4]">${course.price}</span>
          <ProgressCircle percent={course.progress} />
        </div>
      </div>

      <div className="flex gap-3">
        <button className="flex-1 h-12 border border-[#c6c5c8] rounded-2xl text-[16px] font-medium text-black leading-[1.4] hover:bg-[#f9f9f9] transition-colors">
          Save to my list
        </button>
        <button
          disabled={closed}
          className={`flex-1 h-12 rounded-2xl text-[16px] font-medium leading-[1.4] transition-colors ${
            closed
