...
const VerifiedBanner = ({ hasBio = false, hasIntroVideo = false, hasAvailability = false }) => {
  const steps = [hasBio, hasIntroVideo, hasAvailability]
    ...
    <div className="bg-[#121c20] border border-white/[0.13] rounded-3xl px-6 sm:px-11 py-8 sm:py-10 overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8 lg:gap-10">
        <div className="flex flex-col gap-3">
          ...
          <Link to="/tutor/edit-profile" className="h-10 px-3 rounded-[10px] bg-[#007aff] text-sm font-medium text-white self-start flex items-center gap-1.5 mt-3 shadow-[0px_1px_1px_0px_rgba(0,0,0,0.03),inset_0px_3px_3px_0px_rgba(255,255,255,0.12)] hover:bg-[#0066d6] transition-colors no-underline">
        ...
        <div className="flex flex-col items-center gap-2.5 shrink-0 self-center lg:self-auto">
          <div className="relative w-[140px] h-[140px]">
            <svg className="w-[140px] h-[140px] -rotate-90" viewBox="0 0 140 140">
              ...
              <circle cx="70" cy="70" r="60" stroke="#007aff" strokeWidth="8" fill="none" strokeDasharray={circumference} strokeDashoffset={offset} strokeLinecap="round" />
            </svg>
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="text-[26.5px] font-bold text-white leading-none">{done}/{total}</span>
