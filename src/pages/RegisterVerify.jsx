...
const RegisterVerify = () => {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      setError(err?.data?.message || "Invalid or expired verification code")
    }
  }
      ...
      <div className="flex-1 flex items-center justify-center p-4 sm:p-8">
        <div className="w-full max-w-[400px] flex flex-col items-center gap-8">
          <div className="flex flex-col items-center gap-6">
            ...
            <div className="flex flex-col items-center gap-1 text-center">
              ...
              <p className="text-[16px] font-normal text-[#808080] leading-[1.4]">
                Enter the 6-digit code we sent to <span className="font-medium text-black">{email}</span> to finish your sign up.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-6 w-full">
            {error && (
              <div className="bg-[rgba(237,64,48,0.08)] border border-[rgba(237,64,48,0.2)] rounded-xl px-4 py-3 text-sm text-[#df2917]">{error}</div>
            )}
            {resendMsg && (
              <div className="bg-[rgba(64,155,63,0.08)] border border-[rgba(64,155,63,0.2)] rounded-xl px-4 py-3 text-sm text-[#338732]">{resendMsg}</div>
            )}
            <input
              type="text"
              maxLength={6}
              ...
              className={`w-full h-12 px-3 bg-white border rounded-[10px] text-[16px] text-[#181d27] leading-[1.4] outline-none placeholder:text-[#808080] transition-colors ${error ? "border-[#ed4030]" : "border-[#e6e6e6] focus:border-[#007aff]"}`}
            />

            <div className="flex flex-col gap-4 items-center">
              <button disabled={isLoading} onClick={handleVerify} className="w-full h-12 bg-[#0080ff] rounded-full text-[16px] font-medium text-white leading-6 shadow-[0px_1px_3px_0px_rgba(13,13,18,0.05),0px_1px_2px_0px_rgba(13,13,18,0.04)] hover:bg-[#0066dd] transition-colors disabled:opacity-60">
                {isLoading ? "Verifying..." : "Sign up"}
              </button>
              <button disabled={isResending} onClick={handleResend} className="text-[16px] font-medium text-[#0080ff] leading-[1.4] hover:underline disabled:opacity-60">
