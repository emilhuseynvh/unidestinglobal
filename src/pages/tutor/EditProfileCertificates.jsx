...
const EditProfileCertificates = () => {
  ...
  useEffect(() => {
    const list = meData?.data?.tutor?.certifications || meData?.data?.certifications
    if (Array.isArray(list)) {
      setCerts(list.map((url, i) => ({
        id: `cert-${i}`,
        name: decodeURIComponent(url.split("/").pop().replace(/\.[^.]+$/, "")),
        document: url,
      })))
    }
  }, [meData])
  ...
  const handleFileChange = (e) => {
    ...
    if (!file) return
    const entry = { id: Date.now(), name: file.name.replace(/\.[^.]+$/, ""), size: (file.size / (1024 * 1024)).toFixed(1) + " MB", file, progress: 0, done: false }
    setNewFiles((prev) => [...prev, entry])
    const interval = setInterval(() => {
      setNewFiles((prev) => prev.map((c) => {
  ...
  const removeNew = (id) => setNewFiles((prev) => prev.filter((c) => c.id !== id))

  const doneFiles = newFiles.filter((f) => f.done)
  ...
  const handleSave = async () => {
    ...
    const certifications = [...certs.map((c) => c.document), ...doneFiles.map((f) => f.file)]
    try {
      await updateProfile({ certifications }).unwrap()
      toast.success("Certificates saved successfully")
      setNewFiles([])
    } catch (err) {
      toast.error(err?.data?.message || "Failed to save certificates")
    }
  }
        ...
        <div className="bg-white rounded-[18px] p-6 flex flex-col gap-8">
          <div className="flex flex-col gap-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-bold text-[#c3c6cc] uppercase tracking-[1.1px]">Step 5 of 5 — {certs.length > 0 ? "Already complete" : "Not completed"}</span>
            </div>
            <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4">
              <div className="flex flex-col gap-2">
                ...
                <p className="text-sm text-[#5b616d] leading-5 max-w-[460px]">Verified credentials build trust and let you justify higher rates.</p>
              </div>
              {doneFiles.length > 0 && (
          ...
          <div className={`bg-black/[0.03] backdrop-blur-[12px] rounded-3xl p-3 transition-all duration-300 ${showTip ? "max-h-[200px] opacity-100" : "max-h-0 opacity-0 p-0 overflow-hidden"}`}>
            <div className={`bg-white rounded-2xl p-3 flex gap-2 items-start shadow-[0px_1px_1px_0px_rgba(0,0,0,0.03),0px_3px_3px_0px_rgba(0,0,0,0.03),0px_20px_20px_-12px_rgba(0,0,0,0.03)] ${showTip ? "" : "hidden"}`}>
              ...
              <div className="flex flex-col flex-1 min-w-0 px-1">
                ...
                <p className="text-xs sm:text-sm text-[#8c929c] leading-5">Tutors with <span className="font-extrabold text-[#5b616d]">verified credentials</span> charge on average 35% more.</p>
              </div>
          ...
          {certs.map((cert) => (
            <a key={cert.id} href={cert.document} target="_blank" rel="noopener noreferrer" className="bg-white border border-black/[0.06] rounded-[18px] p-[17px] flex items-center gap-4 no-underline cursor-pointer hover:shadow-[0px_3px_12px_rgba(0,0,0,0.06)] transition-shadow">
              ...
              <div className="flex flex-col flex-1 min-w-0">
                ...
                <span className="text-xs text-[#5b616d] leading-4">Uploaded</span>
              </div>
              <span className="bg-[rgba(64,155,63,0.12)] h-5 px-1 rounded-md flex items-center gap-1 shrink-0">
                ...
                <span className="text-[10px] font-medium text-[#338732] leading-4">Uploaded</span>
              </span>
            </a>
          ))}
          ...
          {newFiles.map((cert) => (
            <div key={cert.id} className="bg-white border border-black/[0.06] rounded-[18px] p-[17px] flex items-center gap-4">
              ...
              <div className="flex flex-col gap-1 flex-1 min-w-0">
                <span className="text-sm font-semibold text-[#0a0c11] leading-6 truncate">{cert.name}</span>
                <span className="text-xs text-[#5b616d] leading-4">{cert.size}</span>
                {!cert.done && <div className="h-1.5 bg-[#dddfe4] rounded overflow-hidden"><div className="h-full bg-[#007aff] rounded transition-all duration-100" style={{ width: `${Math.min(cert.progress, 100)}%` }} /></div>}
              </div>
              <button onClick={() => removeNew(cert.id)} className="w-8 h-8 rounded-full border border-black/[0.06] bg-white flex items-center justify-center shrink-0"><svg width="18" height="18" viewBox="0 0 18 18" fill="none"><path d="M13.5 4.5l-9 9M4.5 4.5l9 9" stroke="#8c929c" strokeWidth="1.5" strokeLinecap="round" /></svg></button>
            </div>
          ))}

          <div onClick={() => fileInputRef.current?.click()} className="bg-[#f9f9fa] border border-dashed border-black/[0.06] rounded-xl px-4 py-8 flex flex-col items-center gap-4 cursor-pointer hover:bg-[#f5f5f5] transition-colors">
            <div className="relative">
              <div className="w-[88px] h-[88px] rounded-full bg-[#e8efff] flex items-center justify-center">
                <svg width="40" height="40" viewBox="0 0 40 40" fill="none"><path d="M21 4H9a3 3 0 00-3 3v26a3 3 0 003 3h22a3 3 0 003-3V17l-13-13z" stroke="#3b82f6" strokeWidth="2.5" /><path d="M21 4v13h13" stroke="#3b82f6" strokeWidth="2.5" /><path d="M25 22H15M25 28H15M18 16h-3" stroke="#3b82f6" strokeWidth="2.5" strokeLinecap="round" /></svg>
              </div>
            ...
            <div className="flex flex-col items-center gap-1.5 text-center max-w-[376px]">
              ...
              <p className="text-sm text-[#5b616d] leading-5">Drag & drop your file here, or click to browse.<br />PDF, JPG, PNG (max 6 MB)</p>
            </div>
            <button onClick={(e) => { e.stopPropagation(); fileInputRef.current?.click() }} className="h-12 px-3 rounded-xl bg-[#007aff] text-base font-medium text-white flex items-center gap-2 shadow-[0px_1px_1px_0px_rgba(0,0,0,0.03),inset_0px_3px_3px_0px_rgba(255,255,255,0.12)]">
